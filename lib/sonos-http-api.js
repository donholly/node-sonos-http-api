...
function HttpAPI(discovery, settings) {
  ...
  this.requestHandler = function (req, res) {
    ...
    if (req.url.indexOf('/') === 0) {
        params = req.url.substring(1).split('/');
    } else {
    ...
    winston.info("Sonos request params: " + params);
    ...
    winston.info("Sonos Player: ", player);
  ...
  function handleAction(options, callback) {
    ...
    if (!actions[options.action]) {
      winston.error("No action found for: " + options.action);
